...
#'
#'@param y  points at which the cdf function is comptuted
...
ForecastHMMCdf<-function(y, family, theta, Q, eta, k=1, graph=0){
  ...
  cdf = matrix(0, nrow=length(y), ncol=length(k))

  for (d in 1:length(k)){
    Q_prime = matrixcalc::matrix.power(Q, as.integer(k[d]))
    for (l in 1:r){
      for (j in 1:r){
        cdf[,d] = cdf[,d] + eta[j] * Q_prime[j, l] * CDF(family, y, theta[l,])
      }
    }
  }
  ...
  if (graph != 0){
          ...
          ggplot2::ggplot(df, ggplot2::aes(df$index,df$value)) + ggplot2::geom_line(ggplot2::aes(colour = df$horizon)) +
            ...
            ggplot2::labs(title="Forecasted cumulative density function", x="observations", y="densities" )
          )
  }
